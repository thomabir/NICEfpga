...
# (goal: gain of at least 0.99 at 10 kHz)
...
def plot_bode(f, h, title, filename, annotate=False):
    ...
    if annotate:
        # evaluate which index in f is closest to 10 kHz
        index = np.argmin(np.abs(f-10e3))

        # add multi-line text to the center of the plot
        ax.text(0.5, 0.5, f"At {f[index]/1000:.3f} kHz:\nGain: {magnitude[index]:.4f}\nPhase: {phase[index]:.4f} deg", transform=ax.transAxes, fontsize=10, verticalalignment='center', horizontalalignment='center')
...
plot_bode(f_w, h_differential, 'Bode plot of the Hilbert transformer minus the delay', 'fig/hilbert-bode-differential', annotate=True)
